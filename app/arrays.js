arraysAnswers = {
  ...
  duplicates: function duplicates(arr) {
    ...
    arr.forEach((item) => {
      if (auxArray.indexOf(item) === -1) {
        auxArray.push(item);
      } else if (outcome.indexOf(item) === -1) {
        outcome.push(item);
      }
    });
    return outcome;
   ...
   *
   * @param {Number[]} arr - An array of numbers
   * @param {Number} target - A number to find all occurences to be removed.
   * @returns {Number[]} A new array of numbers avoiding the target number.
   */
  removeWithoutCopy: function removeWithoutCopy(arr, target) {
    while (!arr.every(item => item !== target)) {
